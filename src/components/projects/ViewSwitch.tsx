...
const ViewSwitch: React.FC<Props> = ({ view, onChange }) => {
  return (
    <div className="flex bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden p-1 gap-1">
      ...
      {/* Hide timeline view on mobile/tablet — not optimized for small screens */}
      <button
        onClick={() => onChange("grid")}
        className={`hidden lg:flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg transition ${
          view === "grid"
            ? "bg-purple-600 text-white shadow-sm"
            : "text-gray-500 hover:text-gray-700 hover:bg-gray-50"
        }`}
